## 문제 설명 [(link)](https://school.programmers.co.kr/learn/courses/30/lessons/72410?language=javascript)
...
> #### `replace(/[^\w\.\-]/g, "")`
>
> → 알파벳 소문자, 숫자, 빼기(-), 밑줄(), 마침표(.)를 제외한 모든 문자를 제거

- 특정 문자열 선택

  - `\w` : word. 알파벳 + 숫자 + \_ 중의 한 문자임을 의미한다.
...
- 특정 문자열의 반복
  - `x+` : x 문자가 한번 이상 반복됨
  - `x*` : x 문자가 0번 또는 그 이상 반복됨
  - `x{n}` : x 문자가 n번 반복됨
  - `x{n,}` : x 문자가 n번 이상 반복됨
  - `x{n,m}` : x 문자가 최소 n번 이상 최대 m 번 이하로 반복됨
